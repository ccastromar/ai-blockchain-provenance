...
- OpenTimestamps anchoring provider (`ANCHOR_PROVIDER=ots`): stamps Merkle roots against the public OTS calendars — free, keyless Bitcoin attestation with no crypto custody, eliminating the anchor-wallet threat-model gap for the recommended production configuration. Proofs start pending and are upgraded to confirmed (with the attested Bitcoin block height) by the existing anchor-confirmation cycle; `GET /api/anchors/:id/ots` serves the raw `.ots` proof for offline verification with the official client, receipts carry the provider and proof URL, and `ernest proof verify` prints the OTS chain-of-trust instructions. Interop validated: proofs stamped by Ernest parse and verify in the official Python `ots` tooling.
