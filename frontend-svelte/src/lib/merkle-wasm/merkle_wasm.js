 ...
 * @returns {string}
 */
export function verify_receipt(receipt_json) {
    let deferred2_0;
    let deferred2_1;
    try {
 ...
 * @returns {string}
 */
export function calculate_merkle_root(hashes) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passArrayJsValueToWasm0(hashes, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.calculate_merkle_root(ptr0, len0);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('merkle_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
