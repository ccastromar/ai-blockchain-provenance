// internal/db/repositories/aimodels/mongo_repo.go
package aimodels
...
func (r *mongoRepository) Count(filter bson.D) (int64, error) {
	coll := r.client.Database(r.dbName).Collection("aimodels")
...
func (r *mongoRepository) GetByModelID(modelID string) (map[string]interface{}, error) {
	coll := r.client.Database(r.dbName).Collection("aimodels")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	if err != nil {
		...
		return nil, fmt.Errorf("error finding ai model: %w", err)
	}
	return result, nil
}
