...
Databricks manages registered models, model versions, permissions, aliases, and lifecycle transitions through Unity Catalog. The Ernest connector accepts webhook/polling-style events produced from Unity Catalog / MLflow model registry activity and turns selected lifecycle, alias, serving, and lineage facts into evidence.
...
Current Databricks / Unity Catalog mappings:

| Databricks payload | Ernest incoming event |
| --- | --- |
| model version created / registered | `model.version.created` |
| Champion / Production / prod alias updated | `model.deployed` |
| non-production alias updated | `model.updated` |
| serving endpoint created / updated | `model.deployed` |
| serving endpoint deleted / removed | `model.undeployed` |
| lineage or input table update | `dataset.linked` |
| approval or rejection activity | `model.approved` / `model.rejected` |
